...
- `store`: `localStorage`(キー `genai_passport_store_v1`)に永続化する受験履歴+誤答カウント+中断状態
  ...
  - `store.chapterLaps`: `{章番号: 完走回数}`(2026-08-20追加)。分野別演習は毎回その章の全問を固定順で出題するため、`finishSession()` にたどり着くたび=1周完走とみなして加算する。`chapterLap(ch) = (chapterLaps[ch]||0) + 1` が「現在何周目か」で、ホーム画面の章カード(`.chapLap`)とクイズ画面の `#progressLabel`(分野別演習モードのときだけ `・n周目` を付記)の両方に表示する。中断→再開しても周回数は変わらない(finishSessionを通らないため)
  - **`loadStore()` は未定義フィールドを必ず埋める。** 既存ユーザーのデータには後から追加したフィールドが無いため、増やしたら必ずここに追記すること
  - **`resetStudyData()`** で全部まとめてリセットする対象。新しいフィールドを追加したら忘れずにここにも加えること
