...
結果画面は60問を解かないと出せない。**`inProgress.mock` に60問すべて回答済みの
データを入れる旧テクニックは v1.11.0 で使えなくなった** — 全問回答済みの中断データは
`finishSession()` に流さず破棄して最初からやり直す仕様に変えたため(「100%行ったら
また最初から」の要望による)。

代わりに **59問だけ回答済み**のデータを `inProgress.mock` に入れて模擬試験を開き、
最後の1問を手で答えて「結果を見る」を押す。

## 未対応

- ⚠ **現在の7点は v1.9.3(2026-08-17)時点のもので、UIが古い。** 以降に入った
  以下が写っていないため、申請前に撮り直すこと:
  - 章別の進捗バー(v1.10.0)— `01-home.png`
  - 「‹ 前へ」ボタン(v1.10.0)— `02-quiz.png`
  - 「学習履歴をリセット」ボタン(v1.11.0)— `05-stats.png`
  - 「n周目」表記(v1.12.0)— `01-home.png` / `02-quiz.png`
